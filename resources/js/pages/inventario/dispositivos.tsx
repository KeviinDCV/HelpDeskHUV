...
interface PeripheralType { id: number; name: string; }
...
export default function Dispositivos({ peripherals, states, manufacturers, types, locations, filters }: PeripheralsProps) {
    ...
    const handleSort = (field: string) => {
        ...
        router.get('/inventario/dispositivos', buildFilterParams({ sort: field, direction: newDirection }), { preserveState: false });
    };
    ...
    const handlePerPageChange = (value: string) => {
        router.get('/inventario/dispositivos', buildFilterParams({ per_page: value, page: 1 }), { preserveState: false });
    };
    ...
    const confirmDelete = () => {
        if (deleteModal.id) router.delete(`/inventario/dispositivos/${deleteModal.id}`);
                    ...
                    <div className="bg-white shadow border border-gray-200">
                        ...
                        <div className="px-3 sm:px-6 py-2 sm:py-3 bg-gray-50 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                            ...
                            <p className="text-xs sm:text-sm text-gray-600">
                                <span className="font-medium">{peripherals.data.length}</span> de <span className="font-medium">{peripherals.total}</span>
                            </p>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            <Table>
                                ...
                                <TableBody>
                                    {peripherals.data.map((p) => (
                                        <TableRow key={p.id} className="hover:bg-gray-50">
                                            <TableCell className="font-medium text-xs"><a href={`/inventario/dispositivos/${p.id}`} className="text-[#2c4370] hover:underline">{p.name || '-'}</a></TableCell>
                                            ...
                                            {isAdmin && (
                                                <TableCell className="text-center">
                                                    <div className="flex items-center justify-center gap-1">
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-blue-600 hover:text-blue-800 hover:bg-blue-50" onClick={() => router.visit(`/inventario/dispositivos/${p.id}/editar`)} title="Editar"><Pencil className="h-3.5 w-3.5" /></Button>
                        ...
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t flex flex-col sm:flex-row items-center justify-between gap-3">
                            <div className="text-xs sm:text-sm text-gray-600 order-2 sm:order-1">Página {peripherals.current_page} de {peripherals.last_page}</div>
                            <div className="flex items-center gap-1 sm:gap-2 order-1 sm:order-2 flex-wrap justify-center">
                                {peripherals.links.map((link: PaginationLinks, index: number) => {
                                    ...
                                    if (index === peripherals.links.length - 1) return <Button key={index} variant="outline" size="sm" disabled={!link.url} className="border-[#2c4370] text-[#2c4370] hover:!bg-[#2c4370] hover:!text-white disabled:opacity-50 h-8 w-8 p-0" onClick={() => link.url && router.visit(link.url)}><ChevronRight className="h-4 w-4" /></Button>;
