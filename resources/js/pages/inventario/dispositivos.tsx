...
interface DispositivosProps {
    peripherals: Paginator & { data: Peripheral[] };
    states: Option[];
...
const RUTA = '/inventario/dispositivos';

type Params = Record<string, string | number | undefined>;

// ─── Campos del filtro avanzado ─────────────────────────────────────
// 'otherserial' es el número de inventario de GLPI. Antes se rotulaba "Nombre de usuario
// alternativo", que en GLPI es otro campo (contact): se filtraba una cosa creyendo otra.
...
export default function Dispositivos({ peripherals, states, manufacturers, types, locations, filters }: DispositivosProps) {
    const { auth } = usePage<{ auth: { user: { role: string } } }>().props;
    const isAdmin = auth?.user?.role === 'Administrador';
    const [searchValue, setSearchValue] = React.useState(filters.search || '');
    const [deleteTarget, setDeleteTarget] = React.useState<Peripheral | null>(null);
            ...
            <div className="flex min-h-screen flex-col bg-gray-50">
                ...
                <main className="flex-1">
                    <div className="mx-auto w-full max-w-[1600px] space-y-5 px-4 py-6 sm:px-6">
                        <PageHeader
                            title="Dispositivos"
                            description="Periféricos registrados en el inventario."
                            actions={
                                <>
                        ...
                        <section aria-label="Lista de dispositivos" className="surface-card overflow-hidden">
                            <DataTableToolbar
                                search={searchValue}
                                ...
                                summary={`${peripherals.total.toLocaleString('es-CO')} dispositivos`}
                            />
                            ...
                            <Table className="text-[13px]">
                                <TableHeader>
                                    <TableRow className="hover:bg-transparent">
                                        ...
                                        <SortableHead field="otherserial" label="Nº de inventario" {...orden} />
                                        {isAdmin && (
                                    ...
                                    {peripherals.data.length === 0 ? (
                                        ...
                                        peripherals.data.map((peripheral) => (
                                            <TableRow key={peripheral.id}>
                                                <TableCell>
                                                    <Link href={`${RUTA}/${peripheral.id}`} className="focus-ring rounded font-medium text-huv-ink hover:underline">
                                                        {peripheral.name || '—'}
                                                    </Link>
                                                </TableCell>
                                                <TableCell className="text-gray-600">
                                                    <TruncatedText value={peripheral.entity_name} lines={2} className="max-w-[9rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-700">{peripheral.state_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">{peripheral.manufacturer_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">
                                                    <TruncatedText value={peripheral.location_name} lines={2} className="max-w-[12rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-700">{peripheral.type_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">
                                                    <TruncatedText value={peripheral.model_name} lines={2} className="max-w-[10rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-500 tabular-nums">{formatTableDate(peripheral.date_mod)}</TableCell>
                                                <TableCell className="font-mono text-xs text-gray-700">{peripheral.otherserial || '—'}</TableCell>
                                                {isAdmin && (
                                                    <TableCell className="text-right">
                                                        <div className="flex justify-end gap-0.5">
                                                            <Link
                                                                href={`${RUTA}/${peripheral.id}/editar`}
                                                                aria-label={`Editar ${peripheral.name}`}
                                                                title="Editar"
                                                            ...
                                                            <button
                                                                type="button"
                                                                onClick={() => setDeleteTarget(peripheral)}
                                                                aria-label={`Eliminar ${peripheral.name}`}
                                                                title="Eliminar"
                            ...
                            <DataTablePagination
                                paginator={peripherals}
                                count={peripherals.data.length}
                                noun="dispositivos"
