...
export default function Dispositivos({ peripherals, states, manufacturers, types, locations, filters }: PeripheralsProps) {
    ...
    const go = (params: Record<string, any>) => {
        ...
        router.get('/inventario/dispositivos', limpios, {
            preserveState: false,
    ...
    // ─── Peripheral Field Definitions for Advanced Filter ────────────
    const PERIPHERAL_FIELDS: FieldDef[] = [
    ...
    // ─── Opciones de catálogos para el filtro avanzado (value/label = nombre) ──
                    ...
                    <div className="bg-white shadow border border-gray-200">
                        ...
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-b">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                <h1 className="text-lg sm:text-xl font-semibold text-gray-900">Dispositivos</h1>
                                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white h-9 flex-1 sm:flex-initial" onClick={() => router.visit('/inventario/dispositivos/crear')}>
                        ...
                        <div className="px-3 sm:px-6 py-2 sm:py-3 bg-gray-50 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                            ...
                            <p className="text-xs sm:text-sm text-gray-600">
                                <span className="font-medium">{peripherals.data.length}</span> de{' '}
                                <span className="font-medium">{peripherals.total}</span>
                            </p>
                        </div>
                                            ...
                                            {isAdmin && (
                                                <TableCell className="text-center">
                                                    <div className="flex items-center justify-center gap-1">
                                                        <Button variant="ghost" size="sm" aria-label="Editar" className="h-7 w-7 p-0 text-blue-600 hover:text-blue-800 hover:bg-blue-50" onClick={() => router.visit(`/inventario/dispositivos/${peripheral.id}/editar`)} title="Editar"><Pencil className="h-3.5 w-3.5" /></Button>
                                                        <Button variant="ghost" size="sm" aria-label="Eliminar" className="h-7 w-7 p-0 text-red-600 hover:text-red-800 hover:bg-red-50" onClick={() => handleDelete(peripheral.id, peripheral.name || '')} title="Eliminar"><Trash2 className="h-3.5 w-3.5" /></Button>
                                                    </div>
                                                </TableCell>
                                            )}
                                        </TableRow>
                        ...
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t flex flex-col sm:flex-row items-center justify-between gap-3">
                            <div className="text-xs sm:text-sm text-gray-600 order-2 sm:order-1">
                                Página {peripherals.current_page} de {peripherals.last_page}
                            </div>
                            <div className="flex items-center gap-1 sm:gap-2 order-1 sm:order-2 flex-wrap justify-center">
                                {peripherals.links.map((link: PaginationLinks, index: number) => {
                                    const isMobileVisible = index === 0 || index === peripherals.links.length - 1 || link.active;
                                    if (index === 0) {
                                        return (
