...
interface MonitorType { id: number; name: string; }
...
export default function Monitores({ monitors, states, manufacturers, types, locations, filters }: MonitorsProps) {
    ...
    const handleSort = (field: string) => {
        ...
        router.get('/inventario/monitores', buildFilterParams({ sort: field, direction: newDirection }), { preserveState: false });
    };
    ...
    const clearFilters = () => {
        ...
        setSearchValue('');
        router.get('/inventario/monitores', { per_page: filters.per_page, sort: filters.sort, direction: filters.direction, page: 1 }, { preserveState: false, replace: true });
    };

    const handlePerPageChange = (value: string) => {
        router.get('/inventario/monitores', buildFilterParams({ per_page: value, page: 1 }), { preserveState: false });
    };
    ...
    const confirmDelete = () => {
        if (deleteModal.id) router.delete(`/inventario/monitores/${deleteModal.id}`);
                    ...
                    <div className="bg-white shadow border border-gray-200">
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-b">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                <h1 className="text-lg sm:text-xl font-semibold text-gray-900">Monitores</h1>
                                <div className="flex items-center gap-2">
                                    ...
                                    <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white h-9" onClick={() => router.visit('/inventario/monitores/crear')}>
                        ...
                        <div className="px-3 sm:px-6 py-2 sm:py-3 bg-gray-50 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                            ...
                            <p className="text-xs sm:text-sm text-gray-600"><span className="font-medium">{monitors.data.length}</span> de <span className="font-medium">{monitors.total}</span></p>
                        </div>
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="bg-gray-50">
                                        ...
                                        <TableHead className="font-semibold text-gray-900 text-xs cursor-pointer hover:bg-gray-100" onClick={() => handleSort('serial')}><div className="flex items-center">Número de serie{getSortIcon('serial')}</div></TableHead>
                                ...
                                <TableBody>
                                    {monitors.data.map((m) => (
                                        <TableRow key={m.id} className="hover:bg-gray-50">
                                            <TableCell className="font-medium text-xs"><a href={`/inventario/monitores/${m.id}`} className="text-[#2c4370] hover:underline">{m.name || '-'}</a></TableCell>
                                            <TableCell className="text-xs">{m.entity_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{m.state_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{m.manufacturer_name || '-'}</TableCell>
                                            <TableCell className="font-mono text-xs">{m.serial || '-'}</TableCell>
                                            <TableCell className="text-xs">{m.type_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{m.model_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{m.location_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{m.date_mod ? new Date(m.date_mod).toLocaleString('es-CO', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) : '-'}</TableCell>
                                            {isAdmin && (
                                                <TableCell className="text-center">
                                                    <div className="flex items-center justify-center gap-1">
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-blue-600 hover:text-blue-800 hover:bg-blue-50" onClick={() => router.visit(`/inventario/monitores/${m.id}/editar`)} title="Editar"><Pencil className="h-3.5 w-3.5" /></Button>
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-red-600 hover:text-red-800 hover:bg-red-50" onClick={() => setDeleteModal({ open: true, id: m.id, name: m.name || '' })} title="Eliminar"><Trash2 className="h-3.5 w-3.5" /></Button>
                                                    </div>
                                                </TableCell>
                                            )}
                                        </TableRow>
                        ...
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t flex flex-col sm:flex-row items-center justify-between gap-3">
                            <div className="text-xs sm:text-sm text-gray-600 order-2 sm:order-1">Página {monitors.current_page} de {monitors.last_page}</div>
                            <div className="flex items-center gap-1 sm:gap-2 order-1 sm:order-2 flex-wrap justify-center">
                                {monitors.links.map((link: PaginationLinks, index: number) => {
                                    const isMobileVisible = index === 0 || index === monitors.links.length - 1 || link.active;
                                    ...
                                    if (index === monitors.links.length - 1) return <Button key={index} variant="outline" size="sm" disabled={!link.url} className="border-[#2c4370] text-[#2c4370] hover:!bg-[#2c4370] hover:!text-white disabled:opacity-50 h-8 w-8 p-0" onClick={() => link.url && router.visit(link.url)}><ChevronRight className="h-4 w-4" /></Button>;
