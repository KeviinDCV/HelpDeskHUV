...
interface MonitoresProps {
    monitors: Paginator & { data: Monitor[] };
    states: Option[];
...
const RUTA = '/inventario/monitores';
...
export default function Monitores({ monitors, states, manufacturers, types, locations, filters }: MonitoresProps) {
    const { auth } = usePage<{ auth: { user: { role: string } } }>().props;
    const isAdmin = auth?.user?.role === 'Administrador';
    const [searchValue, setSearchValue] = React.useState(filters.search || '');
    const [deleteTarget, setDeleteTarget] = React.useState<Monitor | null>(null);
            ...
            <div className="flex min-h-screen flex-col bg-gray-50">
                ...
                <main className="flex-1">
                    <div className="mx-auto w-full max-w-[1600px] space-y-5 px-4 py-6 sm:px-6">
                        <PageHeader
                            title="Monitores"
                            description="Pantallas registradas en el inventario."
                            actions={
                                <>
                        ...
                        <section aria-label="Lista de monitores" className="surface-card overflow-hidden">
                            <DataTableToolbar
                                search={searchValue}
                                ...
                                summary={`${monitors.total.toLocaleString('es-CO')} monitores`}
                            />
                                ...
                                <TableBody>
                                    {monitors.data.length === 0 ? (
                                        <DataTableEmpty
                                            ...
                                            title="No hay monitores que coincidan"
                                        ...
                                        monitors.data.map((monitor) => (
                                            <TableRow key={monitor.id}>
                                                <TableCell>
                                                    <Link href={`${RUTA}/${monitor.id}`} className="focus-ring rounded font-medium text-huv-ink hover:underline">
                                                        {monitor.name || '—'}
                                                    </Link>
                                                </TableCell>
                                                <TableCell className="text-gray-600">
                                                    <TruncatedText value={monitor.entity_name} lines={2} className="max-w-[9rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-700">{monitor.state_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">{monitor.manufacturer_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">
                                                    <TruncatedText value={monitor.location_name} lines={2} className="max-w-[12rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-700">{monitor.type_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">
                                                    <TruncatedText value={monitor.model_name} lines={2} className="max-w-[10rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-500 tabular-nums">{formatTableDate(monitor.date_mod)}</TableCell>
                                                <TableCell className="font-mono text-xs text-gray-700">{monitor.otherserial || '—'}</TableCell>
                                                {isAdmin && (
                                                    <TableCell className="text-right">
                                                        <div className="flex justify-end gap-0.5">
                                                            <Link
                                                                href={`${RUTA}/${monitor.id}/editar`}
                                                            ...
                                                            <button
                                                                ...
                                                                aria-label={`Eliminar ${monitor.name}`}
                                                                title="Eliminar"
