...
export default function Monitores({ monitors, states, manufacturers, types, locations, filters }: MonitorsProps) {
    ...
    const go = (params: Record<string, any>) => {
        ...
        router.get('/inventario/monitores', limpios, {
            preserveState: false,
     ...
     * modo que lo configurado en el panel sin haber pulsado "Aplicar filtros" no llegaba a la
    ...
    // ─── Monitor Field Definitions for Advanced Filter ───────────────
                    ...
                    <div className="bg-white shadow border border-gray-200">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                                    <div>
                                        ...
                                        <SearchableSelect
                                            ...
                                            options={[{ value: 'all', label: 'Todos' }, ...(states || []).map((state) => ({ value: state.id.toString(), label: state.name }))]}
                                            placeholder="Todos"
                                    ...
                                    <div>
                                        ...
                                        <SearchableSelect
                                            ...
                                            options={[{ value: 'all', label: 'Todos' }, ...(types || []).map((type) => ({ value: type.id.toString(), label: type.name }))]}
                                            placeholder="Todos"
                                            triggerClassName="h-8 text-xs"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <SearchableSelect
                                            ...
                                            options={[{ value: 'all', label: 'Todas' }, ...(locations || []).map((location) => ({ value: location.id.toString(), label: location.completename || location.name }))]}
                                            placeholder="Todas"
                                            triggerClassName="h-8 text-xs"
                                        />
                                    </div>
                        ...
                        <div className="px-3 sm:px-6 py-2 sm:py-3 bg-gray-50 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                            <div className="flex items-center gap-2 sm:gap-3">
                                ...
                                <Select
                                    ...
                                    onValueChange={(value) => {
                                        router.get('/inventario/monitores', { ...filters, per_page: value }, { preserveState: false })
                                    }}
                                >
                            ...
                            <p className="text-xs sm:text-sm text-gray-600">
                                <span className="font-medium">{monitors.data.length}</span> de{' '}
                                <span className="font-medium">{monitors.total}</span>
                            </p>
                        </div>
                                            ...
                                            {isAdmin && (
                                                <TableCell className="text-center">
                                                    <div className="flex items-center justify-center gap-1">
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-blue-600 hover:text-blue-800 hover:bg-blue-50" onClick={() => router.visit(`/inventario/monitores/${monitor.id}/editar`)} title="Editar" aria-label="Editar">
                                                            <Pencil className="h-3.5 w-3.5" />
                                                        </Button>
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-red-600 hover:text-red-800 hover:bg-red-50" onClick={() => handleDelete(monitor.id, monitor.name || '')} title="Eliminar" aria-label="Eliminar">
                        ...
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t flex flex-col sm:flex-row items-center justify-between gap-3">
                            <div className="text-xs sm:text-sm text-gray-600 order-2 sm:order-1">
                                Página {monitors.current_page} de {monitors.last_page}
                            </div>
