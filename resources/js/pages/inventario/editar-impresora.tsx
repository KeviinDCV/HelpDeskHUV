...
export default function EditarImpresora({ printer, states, manufacturers, types, models, locations, entities, users, groups, existingIps }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.put(`/inventario/impresoras/${printer.id}`, formData, {
    ...
    return (
        <>
            <Head title="Editar Impresora - HelpDesk HUV" />
            <div className="min-h-screen flex flex-col bg-gray-50">
                <GLPIHeader breadcrumb={<div className="flex items-center gap-2 text-sm"><Link href="/dashboard" className="text-gray-600 hover:text-[#2c4370] hover:underline">Inicio</Link><span className="text-gray-400">/</span><Link href="/inventario/global" className="text-gray-600 hover:text-[#2c4370] hover:underline">Inventario</Link><span className="text-gray-400">/</span><Link href="/inventario/impresoras" className="text-gray-600 hover:text-[#2c4370] hover:underline">Impresoras</Link><span className="text-gray-400">/</span><span className="font-medium text-gray-900">Editar</span></div>} />
                <main className="flex-1 px-6 py-6">
                    <div className="max-w-5xl mx-auto">
                        <div className="bg-white rounded-lg shadow">
                            <div className="px-6 py-4 border-b"><h1 className="text-xl font-semibold text-gray-900">Editar Impresora</h1><p className="text-sm text-gray-500 mt-1">Modifique los campos de la impresora</p></div>
                            <form onSubmit={handleSubmit} className="p-4">
                                ...
                                <div className="mb-4"><h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Clasificación</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        ...
                                        <div><Label htmlFor="printertypes_id" className="text-xs">Tipo</Label><Select value={formData.printertypes_id} onValueChange={(v) => setFormData({ ...formData, printertypes_id: v })}><SelectTrigger id="printertypes_id" className="mt-1 h-8 text-xs"><SelectValue placeholder="Seleccionar..." /></SelectTrigger><SelectContent>{types.map((t) => (<SelectItem key={t.id} value={t.id.toString()}>{t.name}</SelectItem>))}</SelectContent></Select></div>
                                        ...
                                        <div><Label htmlFor="printermodels_id" className="text-xs">Modelo</Label><Select value={formData.printermodels_id} onValueChange={(v) => setFormData({ ...formData, printermodels_id: v })}><SelectTrigger id="printermodels_id" className="mt-1 h-8 text-xs"><SelectValue placeholder="Seleccionar..." /></SelectTrigger><SelectContent>{models.map((m) => (<SelectItem key={m.id} value={m.id.toString()}>{m.name}</SelectItem>))}</SelectContent></Select></div>
                                    </div>
                                </div>
                                <div className="mb-4"><h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Ubicación y Responsables</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        ...
                                        <div><Label htmlFor="users_id_tech" className="text-xs">Técnico a cargo</Label><Select value={formData.users_id_tech} onValueChange={(v) => setFormData({ ...formData, users_id_tech: v })}><SelectTrigger id="users_id_tech" className="mt-1 h-8 text-xs"><SelectValue placeholder="Seleccionar..." /></SelectTrigger><SelectContent>{users?.map((u) => (<SelectItem key={u.id} value={u.id.toString()}>{u.name}</SelectItem>))}</SelectContent></Select></div>
                                        <div><Label htmlFor="groups_id_tech" className="text-xs">Grupo a cargo</Label><Select value={formData.groups_id_tech} onValueChange={(v) => setFormData({ ...formData, groups_id_tech: v })}><SelectTrigger id="groups_id_tech" className="mt-1 h-8 text-xs"><SelectValue placeholder="Seleccionar..." /></SelectTrigger><SelectContent>{groups?.map((g) => (<SelectItem key={g.id} value={g.id.toString()}>{g.name}</SelectItem>))}</SelectContent></Select></div>
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-end gap-3 pt-4 border-t"><Button type="button" variant="outline" size="sm" onClick={() => router.visit('/inventario/impresoras')}>Cancelar</Button><Button type="submit" size="sm" disabled={processing} className="bg-[#2c4370] hover:bg-[#3d5583] text-white"><Save className="h-4 w-4 mr-1" />{processing ? 'Guardando...' : 'Guardar'}</Button></div>
                            </form>
                        </div>
                    </div>
                </main>
