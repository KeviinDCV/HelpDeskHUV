...
import { ArrowLeft, Pencil, Network, Ticket } from 'lucide-react';
...
export default function VerDispositivoRed({ networkequipment, tickets }: Props) {
    return (
        <>
            <Head title={`${networkequipment.name} - HelpDesk HUV`} />
            <div className="min-h-screen flex flex-col bg-gray-50">
                <GLPIHeader breadcrumb={
                    <div className="flex items-center gap-2 text-sm">
                        <Link href="/inventario/dispositivos-red" className="text-[#2c4370] hover:underline">Dispositivos de Red</Link>
                        <span className="text-gray-400">/</span>
                        <span className="font-medium text-gray-900">{networkequipment.name}</span>
                    </div>
                } />
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                            <div className="flex items-center gap-3">
                                <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => router.visit('/inventario/dispositivos-red')}
                                >
                                    <ArrowLeft className="h-4 w-4 mr-1" />
                                    Volver
                                </Button>
                                <div>
                                    <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{networkequipment.name}</h1>
                                    <p className="text-sm text-gray-500">ID: {networkequipment.id}</p>
                                </div>
                            </div>
                            <Button
                                onClick={() => router.visit(`/inventario/dispositivos-red/${networkequipment.id}/editar`)}
                                ...
                                <div className="bg-white rounded-lg shadow-sm border p-6">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <p className="text-sm font-medium text-gray-900">{networkequipment.name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Entidad</label>
                                            <p className="text-sm font-medium text-gray-900">{networkequipment.entity_name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Estado</label>
                                            <p className="text-sm font-medium text-gray-900">{networkequipment.state_name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Tipo</label>
                                            <p className="text-sm font-medium text-gray-900">{networkequipment.type_name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Fabricante</label>
                                            <p className="text-sm font-medium text-gray-900">{networkequipment.manufacturer_name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Modelo</label>
                                            <p className="text-sm font-medium text-gray-900">{networkequipment.model_name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Número de Serie</label>
                                            <p className="text-sm font-mono font-medium text-gray-900">{networkequipment.serial || '-'}</p>
                                        </div>
                                        <div>
                                        ...
                                        {networkequipment.ip && (
                                            <div>
                                                ...
                                                <p className="text-sm font-mono font-medium text-gray-900">{networkequipment.ip}</p>
                                            </div>
                                        )}
                                        {networkequipment.mac && (
                                            <div>
                                                <label className="text-xs text-gray-500 uppercase tracking-wide">MAC</label>
                                        ...
                                        {networkequipment.comment && (
                                            <div className="sm:col-span-2">
                                                ...
                                                <p className="text-sm text-gray-900 whitespace-pre-wrap">{networkequipment.comment}</p>
                                            </div>
                                        )}
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Fecha de Creación</label>
                                            <p className="text-sm text-gray-600">
                                                {networkequipment.date_creation 
                                                    ? new Date(networkequipment.date_creation).toLocaleString('es-CO') 
                                                    : '-'}
                                            </p>
                                        </div>
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">
                                                {networkequipment.date_mod 
                                                    ? new Date(networkequipment.date_mod).toLocaleString('es-CO') 
                                                    : '-'}
                                            </p>
                                        </div>
                                    </div>
