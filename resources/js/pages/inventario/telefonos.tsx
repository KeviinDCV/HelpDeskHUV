...
export default function Telefonos({ phones, states, manufacturers, types, locations, filters }: PhonesProps) {
    ...
    const go = (params: Record<string, any>) => {
        ...
        router.get('/inventario/telefonos', limpios, {
            preserveState: false,
    ...
    const confirmDelete = () => {
        if (deleteModal.id) {
            router.delete(`/inventario/telefonos/${deleteModal.id}`);
        }
                    ...
                    <div className="bg-white shadow border border-gray-200">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                ...
                                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white h-9 flex-1 sm:flex-initial" onClick={() => router.visit('/inventario/telefonos/crear')}>
                        ...
                        <div className="px-3 sm:px-6 py-2 sm:py-3 bg-gray-50 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                            ...
                            <p className="text-xs sm:text-sm text-gray-600">
                                <span className="font-medium">{phones.data.length}</span> de{' '}
                                <span className="font-medium">{phones.total}</span>
                            </p>
                        </div>
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="bg-gray-50">
                                        ...
                                        <TableHead
                                            className="font-semibold text-gray-900 text-xs"
                                            aria-sort={filters.sort === 'otherserial' ? (filters.direction === 'asc' ? 'ascending' : 'descending') : 'none'}
                                        >
                                            <button type="button" onClick={() => handleSort('otherserial')} className="flex items-center w-full text-left cursor-pointer hover:text-[#2c4370]">
                                    ...
                                    {phones.data.map((phone) => (
                                        <TableRow key={phone.id} className="hover:bg-gray-50">
                                            <TableCell className="font-medium text-xs">
                                                <a href={`/inventario/telefonos/${phone.id}`} className="text-[#2c4370] hover:underline">
                                                    {phone.name || '-'}
                                                </a>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-xs">{phone.otherserial || '-'}</TableCell>
                                            {isAdmin && (
                                                <TableCell className="text-center">
                                                    <div className="flex items-center justify-center gap-1">
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-blue-600 hover:text-blue-800 hover:bg-blue-50" onClick={() => router.visit(`/inventario/telefonos/${phone.id}/editar`)} title="Editar" aria-label="Editar"><Pencil className="h-3.5 w-3.5" /></Button>
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-red-600 hover:text-red-800 hover:bg-red-50" onClick={() => handleDelete(phone.id, phone.name || '')} title="Eliminar" aria-label="Eliminar"><Trash2 className="h-3.5 w-3.5" /></Button>
                                                    </div>
                                                </TableCell>
                                            )}
                                        </TableRow>
                        ...
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t flex flex-col sm:flex-row items-center justify-between gap-3">
                            <div className="text-xs sm:text-sm text-gray-600 order-2 sm:order-1">
                                Página {phones.current_page} de {phones.last_page}
                            </div>
                            <div className="flex items-center gap-1 sm:gap-2 order-1 sm:order-2 flex-wrap justify-center">
                                {phones.links.map((link: PaginationLinks, index: number) => {
                                    ...
                                    if (index === phones.links.length - 1) {
                                        return (
