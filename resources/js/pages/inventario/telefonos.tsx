...
interface PhoneType { id: number; name: string; }
...
export default function Telefonos({ phones, states, manufacturers, types, locations, filters }: PhonesProps) {
    ...
    const filterFields: FilterFieldDefinition[] = React.useMemo(() => [
        ...
        { value: 'otherserial', label: 'Número de inventario', type: 'text' as const },
    ...
    const handleSort = (field: string) => {
        ...
        router.get('/inventario/telefonos', buildFilterParams({ sort: field, direction: newDirection }), { preserveState: false });
    };
    ...
    const handlePerPageChange = (value: string) => {
        router.get('/inventario/telefonos', buildFilterParams({ per_page: value, page: 1 }), { preserveState: false });
    };
    ...
    const confirmDelete = () => {
        if (deleteModal.id) router.delete(`/inventario/telefonos/${deleteModal.id}`);
        setDeleteModal({ open: false, id: null, name: '' });
    };

    return (
        <>
            <Head title="HelpDesk HUV - Teléfonos" />
            <div className="min-h-screen flex flex-col bg-gray-50">
                <GLPIHeader breadcrumb={
                    <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-gray-400">/</span>
                        <span className="font-medium text-gray-900">Teléfonos</span>
                    </div>
                } />
                    ...
                    <div className="bg-white shadow border border-gray-200">
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-b">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                <h1 className="text-lg sm:text-xl font-semibold text-gray-900">Teléfonos</h1>
                                <div className="flex items-center gap-2">
                                    ...
                                    <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white h-9" onClick={() => router.visit('/inventario/telefonos/crear')}>
                        ...
                        <div className="px-3 sm:px-6 py-2 sm:py-3 bg-gray-50 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                            ...
                            <p className="text-xs sm:text-sm text-gray-600">
                                <span className="font-medium">{phones.data.length}</span> de <span className="font-medium">{phones.total}</span>
                            </p>
                        </div>
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="bg-gray-50">
                                        ...
                                        <TableHead className="font-semibold text-gray-900 text-xs cursor-pointer hover:bg-gray-100" onClick={() => handleSort('otherserial')}><div className="flex items-center">No. Inventario{getSortIcon('otherserial')}</div></TableHead>
                                ...
                                <TableBody>
                                    {phones.data.map((p) => (
                                        <TableRow key={p.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => router.visit(`/inventario/telefonos/${p.id}`)}>
                                            ...
                                            <TableCell className="font-mono text-xs">{p.otherserial || '-'}</TableCell>
                                            ...
                                            {isAdmin && (
                                                <TableCell className="text-center" onClick={(e) => e.stopPropagation()}>
                                                    <div className="flex items-center justify-center gap-1">
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-blue-600 hover:text-blue-800 hover:bg-blue-50" onClick={() => router.visit(`/inventario/telefonos/${p.id}/editar`)} title="Editar"><Pencil className="h-3.5 w-3.5" /></Button>
                        ...
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t flex flex-col sm:flex-row items-center justify-between gap-3">
                            <div className="text-xs sm:text-sm text-gray-600 order-2 sm:order-1">Página {phones.current_page} de {phones.last_page}</div>
                            <div className="flex items-center gap-1 sm:gap-2 order-1 sm:order-2 flex-wrap justify-center">
                                {phones.links.map((link: PaginationLinks, index: number) => {
                                    const isMobileVisible = index === 0 || index === phones.links.length - 1 || link.active;
                                    ...
                                    if (index === phones.links.length - 1) return <Button key={index} variant="outline" size="sm" disabled={!link.url} className="border-[#2c4370] text-[#2c4370] hover:!bg-[#2c4370] hover:!text-white disabled:opacity-50 h-8 w-8 p-0" onClick={() => link.url && router.visit(link.url)}><ChevronRight className="h-4 w-4" /></Button>;
