...
interface TelefonosProps {
    phones: Paginator & { data: Phone[] };
    states: Option[];
...
const RUTA = '/inventario/telefonos';
...
export default function Telefonos({ phones, states, manufacturers, types, locations, filters }: TelefonosProps) {
    const { auth } = usePage<{ auth: { user: { role: string } } }>().props;
    const isAdmin = auth?.user?.role === 'Administrador';
    const [searchValue, setSearchValue] = React.useState(filters.search || '');
    const [deleteTarget, setDeleteTarget] = React.useState<Phone | null>(null);
            ...
            <div className="flex min-h-screen flex-col bg-gray-50">
                ...
                <main className="flex-1">
                    <div className="mx-auto w-full max-w-[1600px] space-y-5 px-4 py-6 sm:px-6">
                        <PageHeader
                            title="Teléfonos"
                            description="Teléfonos registrados en el inventario."
                            actions={
                                <>
                        ...
                        <section aria-label="Lista de teléfonos" className="surface-card overflow-hidden">
                            <DataTableToolbar
                                search={searchValue}
                                ...
                                summary={`${phones.total.toLocaleString('es-CO')} teléfonos`}
                            />
                                ...
                                <TableBody>
                                    {phones.data.length === 0 ? (
                                        <DataTableEmpty
                                            ...
                                            title="No hay teléfonos que coincidan"
                                        ...
                                        phones.data.map((phone) => (
                                            <TableRow key={phone.id}>
                                                <TableCell>
                                                    <Link href={`${RUTA}/${phone.id}`} className="focus-ring rounded font-medium text-huv-ink hover:underline">
                                                        {phone.name || '—'}
                                                    </Link>
                                                </TableCell>
                                                <TableCell className="text-gray-600">
                                                    <TruncatedText value={phone.entity_name} lines={2} className="max-w-[9rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-700">{phone.state_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">{phone.manufacturer_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">
                                                    <TruncatedText value={phone.location_name} lines={2} className="max-w-[12rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-700">{phone.type_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">
                                                    <TruncatedText value={phone.model_name} lines={2} className="max-w-[10rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-500 tabular-nums">{formatTableDate(phone.date_mod)}</TableCell>
                                                <TableCell className="font-mono text-xs text-gray-700">{phone.otherserial || '—'}</TableCell>
                                                {isAdmin && (
                                                    <TableCell className="text-right">
                                                        <div className="flex justify-end gap-0.5">
                                                            <Link
                                                                href={`${RUTA}/${phone.id}/editar`}
                                                                aria-label={`Editar ${phone.name}`}
                                                                title="Editar"
                                                            ...
                                                            <button
                                                                ...
                                                                aria-label={`Eliminar ${phone.name}`}
                                                                title="Eliminar"
