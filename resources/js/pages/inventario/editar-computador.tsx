...
export default function EditarComputador({ computer, states, manufacturers, types, models, locations, entities }: Props) {
    const [formData, setFormData] = useState({
        name: computer.name || '',
        serial: computer.serial || '',
        otherserial: computer.otherserial || '',
        states_id: computer.states_id ? computer.states_id.toString() : '',
        manufacturers_id: computer.manufacturers_id ? computer.manufacturers_id.toString() : '',
        computertypes_id: computer.computertypes_id ? computer.computertypes_id.toString() : '',
        computermodels_id: computer.computermodels_id ? computer.computermodels_id.toString() : '',
        locations_id: computer.locations_id ? computer.locations_id.toString() : '',
        entities_id: computer.entities_id ? computer.entities_id.toString() : '',
    ...
    return (
        <>
            <Head title="Editar Computador - HelpDesk HUV" />
            <div className="min-h-screen flex flex-col bg-gray-50">
                <GLPIHeader breadcrumb={
                    <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-gray-400">/</span>
                        <button onClick={() => router.visit('/inventario/computadores')} className="text-gray-600 hover:text-gray-900">Computadores</button>
                        <span className="text-gray-400">/</span>
                    ...
                    <div className="max-w-5xl mx-auto">
                        <div className="bg-white rounded-lg shadow">
                            <div className="px-6 py-4 border-b">
                                <h1 className="text-xl font-semibold text-gray-900">Editar Computador</h1>
                                <p className="text-sm text-gray-500 mt-1">Modifique los campos del equipo</p>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="p-4">
                                <div className="mb-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        <div className="md:col-span-2">
                                            ...
                                            <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required className="mt-1 h-8 text-sm" />
                                        ...
                                        <div>
                                            ...
                                            <Input id="serial" value={formData.serial} onChange={(e) => setFormData({ ...formData, serial: e.target.value })} className="mt-1 h-8 text-sm" />
                                        </div>
                                        <div>
                                            <Label htmlFor="otherserial" className="text-xs">Nº Inventario</Label>
                                            <Input id="otherserial" value={formData.otherserial} onChange={(e) => setFormData({ ...formData, otherserial: e.target.value })} className="mt-1 h-8 text-sm" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mb-4">
                                    ...
                                    <Textarea id="comment" value={formData.comment} onChange={(e) => setFormData({ ...formData, comment: e.target.value })} rows={2} className="text-sm" />
                                </div>

                                <div className="flex justify-end gap-3 pt-4 border-t">
                                    <Button type="button" variant="outline" size="sm" onClick={() => router.visit('/inventario/computadores')}>Cancelar</Button>
