...
interface NetworkEquipmentType { id: number; name: string; }
...
export default function DispositivosRed({ networkequipments, states, manufacturers, types, locations, filters }: NetworkEquipmentsProps) {
    ...
    const handleSort = (field: string) => {
        ...
        router.get('/inventario/dispositivos-red', buildFilterParams({ sort: field, direction: newDirection }), { preserveState: false });
    };
    ...
    const clearFilters = () => {
        ...
        setSearchValue('');
        router.get('/inventario/dispositivos-red', { per_page: filters.per_page, sort: filters.sort, direction: filters.direction, page: 1 }, { preserveState: false, replace: true });
    };

    const handlePerPageChange = (value: string) => {
        router.get('/inventario/dispositivos-red', buildFilterParams({ per_page: value, page: 1 }), { preserveState: false });
    };
    ...
    const confirmDelete = () => {
        if (deleteModal.id) router.delete(`/inventario/dispositivos-red/${deleteModal.id}`);
                    ...
                    <div className="bg-white shadow border border-gray-200">
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-b">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white h-9" onClick={() => router.visit('/inventario/dispositivos-red/crear')}>
                        ...
                        <div className="px-3 sm:px-6 py-2 sm:py-3 bg-gray-50 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                            ...
                            <p className="text-xs sm:text-sm text-gray-600">
                                <span className="font-medium">{networkequipments.data.length}</span> de <span className="font-medium">{networkequipments.total}</span>
                            </p>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            <Table>
                                ...
                                <TableBody>
                                    {networkequipments.data.map((eq) => (
                                        <TableRow key={eq.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => router.visit(`/inventario/dispositivos-red/${eq.id}`)}>
                                            <TableCell className="font-medium text-xs text-[#2c4370] hover:underline">
                                                {eq.name || '-'}
                                            </TableCell>
                                            <TableCell className="text-xs">{eq.entity_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{eq.state_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{eq.manufacturer_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{eq.location_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{eq.type_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{eq.model_name || '-'}</TableCell>
                                            <TableCell className="text-xs">{eq.date_mod ? new Date(eq.date_mod).toLocaleString('es-CO', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) : '-'}</TableCell>
                                            {isAdmin && (
                                                <TableCell className="text-center" onClick={(e) => e.stopPropagation()}>
                                                    <div className="flex items-center justify-center gap-1">
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-blue-600 hover:text-blue-800 hover:bg-blue-50" onClick={() => router.visit(`/inventario/dispositivos-red/${eq.id}/editar`)} title="Editar"><Pencil className="h-3.5 w-3.5" /></Button>
                                                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-red-600 hover:text-red-800 hover:bg-red-50" onClick={() => setDeleteModal({ open: true, id: eq.id, name: eq.name || '' })} title="Eliminar"><Trash2 className="h-3.5 w-3.5" /></Button>
                                                    </div>
                                                </TableCell>
                                            )}
                                        </TableRow>
                        ...
                        <div className="px-3 sm:px-6 py-3 sm:py-4 border-t flex flex-col sm:flex-row items-center justify-between gap-3">
                            <div className="text-xs sm:text-sm text-gray-600 order-2 sm:order-1">Página {networkequipments.current_page} de {networkequipments.last_page}</div>
                            <div className="flex items-center gap-1 sm:gap-2 order-1 sm:order-2 flex-wrap justify-center">
                                {networkequipments.links.map((link: PaginationLinks, index: number) => {
                                    ...
                                    if (index === networkequipments.links.length - 1) return <Button key={index} variant="outline" size="sm" disabled={!link.url} className="border-[#2c4370] text-[#2c4370] hover:!bg-[#2c4370] hover:!text-white disabled:opacity-50 h-8 w-8 p-0" onClick={() => link.url && router.visit(link.url)}><ChevronRight className="h-4 w-4" /></Button>;
            ...
            <Dialog open={deleteModal.open} onOpenChange={(open) => setDeleteModal({ ...deleteModal, open })}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription className="pt-2">¿Está seguro de eliminar el dispositivo de red <span className="font-semibold text-gray-900">"{deleteModal.name}"</span>? Esta acción no se puede deshacer.</DialogDescription>
                    </DialogHeader>
