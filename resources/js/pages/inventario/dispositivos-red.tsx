...
interface DispositivosRedProps {
    networkequipments: Paginator & { data: NetworkEquipment[] };
    states: Option[];
...
const RUTA = '/inventario/dispositivos-red';

type Categoria = 'all' | 'switches' | 'wifi';
...
export default function DispositivosRed({ networkequipments, states, manufacturers, types, locations, filters }: DispositivosRedProps) {
    const { auth } = usePage<{ auth: { user: { role: string } } }>().props;
    const isAdmin = auth?.user?.role === 'Administrador';
    const [searchValue, setSearchValue] = React.useState(filters.search || '');
    const [deleteTarget, setDeleteTarget] = React.useState<NetworkEquipment | null>(null);
    const [deleting, setDeleting] = React.useState(false);
    const [showFilters, setShowFilters] = React.useState(false);
    const [deviceCategory, setDeviceCategory] = React.useState<Categoria>((filters.device_category as Categoria) || 'all');
    ...
    const buildParams = (overrides: Params = {}): Params => {
        ...
        // Panel "Filtros" (filtros básicos) + pestañas de categoría
        if (searchValue) params.search = searchValue;
        if (deviceCategory !== 'all') params.device_category = deviceCategory;
    ...
    const clearFilters = () => {
        ...
        go({
            ...
            direction: filters.direction,
            page: 1,
            ...(deviceCategory !== 'all' ? { device_category: deviceCategory } : {}),
    ...
    return (
        <>
                ...
                <main className="flex-1">
                    <div className="mx-auto w-full max-w-[1600px] space-y-5 px-4 py-6 sm:px-6">
                        <PageHeader
                            ...
                            description="Switches, platos WiFi y demás equipos de red registrados en el inventario."
                            actions={
                                <>
                        ...
                        <SegmentedControl<Categoria>
                            label="Categoría de equipo"
                            value={deviceCategory}
                            onChange={(cat) => {
                                // Se pasa como override: setDeviceCategory aún no se refleja en buildParams().
                                setDeviceCategory(cat);
                                go(buildParams({ device_category: cat === 'all' ? undefined : cat }));
                            }}
                            options={[
                                { value: 'all', label: 'Todos' },
                                { value: 'switches', label: 'Switches', icon: <Network aria-hidden="true" /> },
                                { value: 'wifi', label: 'Platos WiFi', icon: <Wifi aria-hidden="true" /> },
                            ]}
                        />

                        <section aria-label="Lista de dispositivos de red" className="surface-card overflow-hidden">
                            <DataTableToolbar
                                search={searchValue}
                                ...
                                summary={`${networkequipments.total.toLocaleString('es-CO')} dispositivos`}
                            />
                            ...
                            <Table className="text-[13px]">
                                ...
                                <TableBody>
                                    {networkequipments.data.length === 0 ? (
                                        <DataTableEmpty
                                            colSpan={isAdmin ? 9 : 8}
                                            title="No hay dispositivos que coincidan"
                                        ...
                                        networkequipments.data.map((equipment) => (
                                            <TableRow key={equipment.id}>
                                                <TableCell>
                                                    <Link href={`${RUTA}/${equipment.id}`} className="focus-ring rounded font-medium text-huv-ink hover:underline">
                                                        {equipment.name || '—'}
                                                    </Link>
                                                </TableCell>
                                                <TableCell className="text-gray-600">
                                                    <TruncatedText value={equipment.entity_name} lines={2} className="max-w-[9rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-700">{equipment.state_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">{equipment.manufacturer_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">
                                                    <TruncatedText value={equipment.location_name} lines={2} className="max-w-[12rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-700">{equipment.type_name || '—'}</TableCell>
                                                <TableCell className="text-gray-700">
                                                    <TruncatedText value={equipment.model_name} lines={2} className="max-w-[10rem]" />
                                                </TableCell>
                                                <TableCell className="text-gray-500 tabular-nums">{formatTableDate(equipment.date_mod)}</TableCell>
                                                {isAdmin && (
                                                    <TableCell className="text-right">
                                                        <div className="flex justify-end gap-0.5">
                                                            <Link
                                                                href={`${RUTA}/${equipment.id}/editar`}
                            ...
                            <DataTablePagination
                                paginator={networkequipments}
                                count={networkequipments.data.length}
                                noun="dispositivos"
            ...
            <ConfirmDialog
                ...
                title="Eliminar dispositivo de red"
                description={
                    <>
