...
import { ArrowLeft, Pencil, Monitor, Package, Cpu, Ticket } from 'lucide-react';
...
export default function VerComputador({ computer, monitors, software, peripherals, tickets }: Props) {
    return (
        <>
                    ...
                    <div className="max-w-6xl mx-auto">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div className="lg:col-span-2 space-y-6">
                                <div className="bg-white rounded-lg shadow-sm border p-6">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm font-medium text-gray-900">{computer.entity_name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Estado</label>
                                            <p className="text-sm font-medium text-gray-900">{computer.state_name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Tipo</label>
                                            <p className="text-sm font-medium text-gray-900">{computer.type_name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Fabricante</label>
                                            <p className="text-sm font-medium text-gray-900">{computer.manufacturer_name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Modelo</label>
                                            <p className="text-sm font-medium text-gray-900">{computer.model_name || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Número de Serie</label>
                                            <p className="text-sm font-mono font-medium text-gray-900">{computer.serial || '-'}</p>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Número de Inventario</label>
                                            <p className="text-sm font-mono font-medium text-gray-900">{computer.otherserial || '-'}</p>
                                        </div>
                                        <div className="sm:col-span-2">
                                            <label className="text-xs text-gray-500 uppercase tracking-wide">Localización</label>
                                            <p className="text-sm font-medium text-gray-900">{computer.location_name || '-'}</p>
                                        </div>
                                        {computer.comment && (
                                            <div className="sm:col-span-2">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">
                                                {computer.date_creation 
                                                    ? new Date(computer.date_creation).toLocaleString('es-CO') 
                                                    : '-'}
                                            </p>
                                        </div>
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">
                                                {computer.date_mod 
                                                    ? new Date(computer.date_mod).toLocaleString('es-CO') 
                                                    : '-'}
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div className="bg-white rounded-lg shadow-sm border p-6">
                                    <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                                        ...
                                        Software Instalado
                                        <span className="text-sm font-normal text-gray-500">({software.length})</span>
                                    </h2>
                                    {software.length > 0 ? (
                                        <div className="max-h-64 overflow-y-auto">
                                            <table className="w-full text-sm">
                                                <thead className="bg-gray-50 sticky top-0">
                                                    <tr>
                                                        <th className="text-left py-2 px-3 text-xs font-medium text-gray-500 uppercase">Nombre</th>
                                                        <th className="text-left py-2 px-3 text-xs font-medium text-gray-500 uppercase">Versión</th>
                                                    </tr>
                                                </thead>
                                        ...
                                        <p className="text-sm text-gray-500 text-center py-4">No hay software registrado</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="bg-white rounded-lg shadow-sm border p-6">
                                    <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                                        <Monitor className="h-5 w-5 text-[#2c4370]" />
                                        Monitores
                                        <span className="text-sm font-normal text-gray-500">({monitors.length})</span>
                                    </h2>
                                    {monitors.length > 0 ? (
                                        <div className="space-y-3">
                                            {monitors.map((monitor) => (
                                                <div key={monitor.id} className="p-3 bg-gray-50 rounded-lg">
                                                    <p className="font-medium text-gray-900 text-sm">{monitor.name}</p>
                                                    {monitor.manufacturer_name && (
                                                        <p className="text-xs text-gray-500">{monitor.manufacturer_name}</p>
                                                    )}
                                                    {monitor.serial && (
                                                        <p className="text-xs font-mono text-gray-500">S/N: {monitor.serial}</p>
                                                    )}
                                                </div>
                                            ))}
                                        </div>
                                    ) : (
                                        <p className="text-sm text-gray-500 text-center py-2">Sin monitores conectados</p>
                                    )}
                                </div>
                                ...
                                {peripherals.length > 0 && (
                                    <div className="bg-white rounded-lg shadow-sm border p-6">
                                        <h2 className="text-lg font-semibold text-gray-900 mb-4">
                                            Periféricos ({peripherals.length})
                                        </h2>
                                        <div className="space-y-2">
                                            {peripherals.map((peripheral) => (
                                                <div key={peripheral.id} className="p-2 bg-gray-50 rounded text-sm">
                                                    <p className="font-medium text-gray-900">{peripheral.name}</p>
                                                    {peripheral.serial && (
                                                        <p className="text-xs font-mono text-gray-500">S/N: {peripheral.serial}</p>
                                                    )}
                                                </div>
                                            ))}
                                        </div>
