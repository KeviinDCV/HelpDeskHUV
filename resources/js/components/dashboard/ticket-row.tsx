...
const ESTADO: Record<number, { corto: string; punto: string }> = {
...
export function PriorityPill({ priority, name }: { priority: number; name: string }) {
    ...
    return (
        <span className={`inline-flex items-center rounded-md px-1.5 py-0.5 text-[11px] font-semibold leading-4 ${p.pill}`}>
            {name}
        </span>
    );
}
...
export function StatusPill({ status, name }: { status: number; name: string }) {
    ...
    return (
        <span title={name} className="inline-flex items-center gap-1.5 rounded-md bg-gray-100 px-1.5 py-0.5 text-[11px] font-medium leading-4 text-gray-700">
